...
void clear_rdopt ()
{
  ...
  {
    free_mem_ACcoeff (cofAC_8x8CbCrts[0]);
    free_mem_ACcoeff (cofAC_8x8CbCrts[1]);
...
void init_rdopt ()
{
  ...
  //if (img->yuv_format==YUV444 && !IS_INDEPENDENT(input))
  {
    get_mem_ACcoeff (&cofAC_8x8CbCrts[0]);
    get_mem_ACcoeff (&cofAC_8x8CbCrts[1]);
                                  ...
                                  int c_nzCbCr[2])
{
  ...
  *nonzero = pDCT_4x4 (currMB, PLANE_Y, block_x, block_y, &dummy, 1);
  ...
  if((active_sps->chroma_format_idc == YUV444) && (IS_INDEPENDENT(input)==0))
  {
    ...
    {
      select_plane(k);
      c_nzCbCr[k - 1] = pDCT_4x4(currMB, k, block_x, block_y, &dummy, 1);
  ...
  if (input->symbol_mode == CAVLC)
  {
    ...
    if((active_sps->chroma_format_idc == YUV444) && (IS_INDEPENDENT(input)==0) )  
    {
      ...
      rate  += writeCoeff4x4_CAVLC (currMB, CR1, b8, b4, 0);
    }
  }
  else
  {
 ...
 *************************************************************************************
 */
int Mode_Decision_for_4x4IntraBlocks (Macroblock *currMB, int  b8,  int  b4,  double  lambda,  double*  min_cost, int cr_cbp[2])
{
  ...
  *min_cost = INT_MAX;
  ...
  intrapred_4x4 (PLANE_Y, pic_pix_x, pic_pix_y, &left_available, &up_available, &all_available);

  if ((img->yuv_format == YUV444) && !IS_INDEPENDENT(input))
  {
    select_plane(PLANE_U);
    intrapred_4x4 (PLANE_U, pic_pix_x, pic_pix_y, &left_available, &up_available, &all_available);
    select_plane(PLANE_V);
    intrapred_4x4 (PLANE_V, pic_pix_x, pic_pix_y, &left_available, &up_available, &all_available);
    select_plane(PLANE_Y);
  }
    ...
    if( available_mode)
    {
      ...
      else
      {
        ...
        if ((img->yuv_format == YUV444) && !IS_INDEPENDENT(input))
          for (j=0; j<4; j++)
          {
            memcpy(&img->mpr[1][block_y+j][block_x], img->mpr_4x4[1][ipmode][j], BLOCK_SIZE * sizeof(imgpel));
            memcpy(&img->mpr[2][block_y+j][block_x], img->mpr_4x4[2][ipmode][j], BLOCK_SIZE * sizeof(imgpel));
            for (i=0; i<4; i++)
            {
             img->m7[1][block_y+j][block_x+i] = (int) (imgUV_org[0][pic_opix_y+j][pic_opix_x+i] - img->mpr_4x4[1][ipmode][j][i]);
             img->m7[2][block_y+j][block_x+i] = (int) (imgUV_org[1][pic_opix_y+j][pic_opix_x+i] - img->mpr_4x4[2][ipmode][j][i]);
            }
          }
...
#endif
        if ((rdcost = RDCost_for_4x4IntraBlocks (&c_nz, b8, b4, ipmode, lambda, mostProbableMode, c_nzCbCr)) < min_rdcost)
        {
          //--- set coefficients ---
  ...
  if (!input->rdopt)
  {
    ...
    if ((img->yuv_format == YUV444) && !IS_INDEPENDENT(input))
    {
      ...
      for (k = PLANE_U; k <= PLANE_V; k++)
      {
        ...
        cr_cbp[k-1] = pDCT_4x4(currMB, k, block_x,block_y,&dummy,1);
      }
      select_plane(PLANE_Y);
    }
  }
  else
  {
    if( (img->yuv_format==YUV444) && !IS_INDEPENDENT(input) )
    {
      ...
      cr_cbp[0] = pDCT_4x4(currMB, PLANE_U, block_x,block_y,&dummy,1);
      select_plane(PLANE_V);
      for (j=0; j<4; j++)
      {
        for (i=0; i<4; i++)
      ...
      cr_cbp[1]  = pDCT_4x4(currMB, PLANE_V,block_x,block_y,&dummy,1);
      select_plane(PLANE_Y);
    }
                             ...
                             short   l1_ref)     // <-- L1 reference picture
{
  ...
  if( img->yuv_format==YUV444 && !IS_INDEPENDENT(input) ) 
  {
    *cnt_nonz += ( coeff_cost1 + coeff_cost2 );
  }
  //===== get residue =====
...
void Intra16x16_Mode_Decision (Macroblock* currMB, int* i16mode)
{
  ...
  intrapred_16x16 (PLANE_Y);
  if ((active_sps->chroma_format_idc == YUV444)&&(IS_INDEPENDENT(input)==0))
  {
    select_plane(PLANE_U);
    intrapred_16x16 (PLANE_U);
    select_plane(PLANE_V);
    intrapred_16x16 (PLANE_V);
    select_plane(PLANE_Y);
  }
  ...
  {
    select_plane(PLANE_U);
    cmp_cbp[0] = dct_16x16 (currMB, PLANE_U, *i16mode);
    select_plane(PLANE_V);
    cmp_cbp[1] = dct_16x16 (currMB, PLANE_V, *i16mode);
    select_plane(PLANE_Y);
    currMB->cbp |= cmp_cbp[0];
    currMB->cbp |= cmp_cbp[1];
    cmp_cbp[0] = currMB->cbp;
...
void SetCoeffAndReconstruction8x8 (Macroblock* currMB)
{
  ...
  if (currMB->luma_transform_size_8x8_flag)
  {
    ...
    if( img->yuv_format == YUV444 && !IS_INDEPENDENT(input) )
    {
      for (uv=0; uv<2; uv++)
      {
        for (block = 0; block<4; block++)
        {
          for (k = 0; k<4; k++)
            for (j = 0; j<2; j++)
              memcpy (img->cofAC[4+block+uv*4][k][j],cofAC_8x8CbCrts[uv][block][k][j], 65 * sizeof(int));
        }
      }
    }
    ...
    else
    {
      ...
      if( img->yuv_format == YUV444 && !IS_INDEPENDENT(input) ) 
      {
        cmp_cbp[0] = cmp_cbp[1] = cbp8_8x8ts;
        for (j = 0; j < MB_BLOCK_SIZE; j++)
        {
                            ...
                            int i16mode )
{
  ...
  if (mode<P8x8)
  {
    ...
    if (img->yuv_format==YUV444 && !IS_INDEPENDENT(input))      
    {
      if(mode==0 && (currMB->cbp==0 && cmp_cbp[0] == 0 && cmp_cbp[1] == 0)&& currMB->luma_transform_size_8x8_flag == 1) //for B_skip, luma_transform_size_8x8_flag=0 only
        return 0;
    }
    else
    {
...
void store_macroblock_parameters (Macroblock *currMB, int mode)
{
  ...
  if (mode || bframe)
  {
    ...
    cbp     = currMB->cbp;
    curr_cbp[0] = cmp_cbp[0];  
    curr_cbp[1] = cmp_cbp[1]; 
...
void set_stored_macroblock_parameters (Macroblock *currMB)
{
  ...
  cmp_cbp[0] = curr_cbp[0]; 
  cmp_cbp[1] = curr_cbp[1]; 
  currMB->cbp |= cmp_cbp[0];
  currMB->cbp |= cmp_cbp[1];
  cmp_cbp[0] = currMB->cbp; 
  cmp_cbp[1] = currMB->cbp;
  ...
  if (img->yuv_format==YUV444 && !IS_INDEPENDENT(input))
  {
    if (((currMB->cbp == 0) && cmp_cbp[0] == 0 && cmp_cbp[1] == 0) && !(IS_OLDINTRA(currMB) || currMB->mb_type == I8MB))
      currMB->luma_transform_size_8x8_flag = 0;
    else
