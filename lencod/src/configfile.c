...
static void PatchInp (void)
{
  ...
  // Cabac/CAVLC consistency check
  if (input->symbol_mode != CAVLC && input->symbol_mode != CABAC)
  {
    snprintf (errortext, ET_SIZE, "Unsupported symbol mode=%d, use CAVLC=0 or CABAC=1",input->symbol_mode);
    error (errortext, 400);
  }
...
static void ProfileCheck(void)
{
  ...
  if ( input->ProfileIDC == FREXT_CAVLC444 )
  {
    ...
    {
      snprintf(errortext, ET_SIZE, "\nCABAC is not allowed in FREXT_CAVLC444.");
      error (errortext, 500);
    }
