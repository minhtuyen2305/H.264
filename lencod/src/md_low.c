...
void encode_one_macroblock_low (Macroblock *currMB)
{
  ...
  if (!intra)
  {
    ...
    if ((!inter_skip) && enc_mb.valid[P8x8])
    {
      ...
      if (input->Transform8x8Mode)
      {
        ...
        {
          submacroblock_mode_decision(enc_mb, &tr8x8, currMB, cofAC_8x8ts[block], cofAC_8x8CbCrts[0][block], cofAC_8x8CbCrts[1][block],
  ...
  if (enc_mb.valid[I16MB]) // check INTRA16x16
  {
    ...
    rd_cost = find_sad_16x16 (&i16mode);
  ...
  //---------------------------------------------------------------------------
  {
    ...
    else
    {
      ...
      if ((best_mode!=I4MB)&&(best_mode != I8MB))
      {
        ...
        if (best_mode!=I16MB)
        {
          ...
          if ((img->yuv_format == YUV444) && !IS_INDEPENDENT(input))
          {
            if((currMB->cbp==0 && cmp_cbp[0] == 0 && cmp_cbp[1] == 0) &&(best_mode==0))
              currMB->luma_transform_size_8x8_flag = 0;
          }
    ...
    if((img->yuv_format == YUV444) && !IS_INDEPENDENT(input))
    {
      if ((pslice) && best_mode==1 && currMB->cbp==0 && cmp_cbp[0] == 0 && cmp_cbp[1] == 0 &&
